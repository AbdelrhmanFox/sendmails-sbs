...
| **Trainer Classroom** | Classroom includes shareable participant links, assignment submissions/review, trainer-side multi-file assignment attachments, course-mapped access control for trainers, and the React **Training → Assignments** page for staff assignment CRUD and inline submission review (`classroom-data` including `pending-submissions`, `public-classroom`, `classroom-assignment-upload`). |
...
**Classroom and reassigning a batch to another course:** The staff Classroom loads authoritative `course_id` via `classroom-data?resource=batch-context` whenever the room is refreshed (including after saving a batch in Operations). **Course library** links use that id. If a non-admin trainer is **not** named on the batch `trainer` field, they must have **`trainer_course_access`** for the batch’s **current** `course_id`; after moving the batch to a different course, add a mapping for the new course or classroom calls return 403 until they do.

**Admin delete (dashboard + API):** `DELETE` `/.netlify/functions/operations-data?entity=<courses|batches|trainees|enrollments>&id=<row_uuid>` is allowed only for JWT role `admin`. It removes dependent **payments** and **enrollments** first (payments reference enrollments with `ON DELETE RESTRICT`). Deleting a **course** deletes all its batches (and their classroom rows via FK cascade) then the course (course library and trainer access rows cascade on course). The Operations UI shows **Delete** on course/batch rows and per-trainee / **Delete selected** for trainees when logged in as admin.
