...
export function OperationsPage() {
  ...
  return (
    <div className="space-y-4">
      {/* Tabs + actions */}
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--brand-border)] pb-0">
        ...
        <div className="flex items-center gap-2 pb-1">
          <Button variant="secondary" size="sm" type="button" onClick={() => navigate('/operations/import')}>
            Import Excel
          </Button>
      ...
      {/* Compact toolbar */}
      <div className="flex flex-wrap items-center gap-2 rounded-[var(--brand-radius)] border border-[var(--brand-border)] bg-[var(--brand-surface)] px-4 py-2.5">
        <div className="relative flex-1 min-w-[180px] max-w-xs">
          <svg className="absolute left-2.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-[var(--brand-muted)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
          </svg>
          <input
            className="w-full rounded-[var(--brand-radius-dense)] border border-[var(--brand-border)] bg-[var(--brand-surface-2)] py-1.5 pl-8 pr-3 text-sm text-[var(--brand-text)]"
            placeholder="Search…"
            value={searchQuery}
        ...
        <button
          ...
          className={`rounded-[var(--brand-radius-dense)] border px-3 py-1.5 text-xs font-medium transition-colors ${
            clientFilter !== 'all'
              ? 'border-[var(--brand-primary)]/40 bg-[var(--brand-primary-subtle)] text-[var(--brand-primary-2)]'
              : 'border-[var(--brand-border)] text-[var(--brand-muted)] hover:text-[var(--brand-text)]'
          }`}
        >
          {clientFilter === 'all' ? 'All statuses' : clientFilter === 'active' ? 'Active only' : 'Inactive only'}
        </button>
        <button
          type="button"
          onClick={() => setSortDir((prev) => (prev === 'asc' ? 'desc' : 'asc'))}
          className="rounded-[var(--brand-radius-dense)] border border-[var(--brand-border)] px-3 py-1.5 text-xs font-medium text-[var(--brand-muted)] hover:text-[var(--brand-text)]"
        >
          {sortDir === 'asc' ? '↑ A–Z' : '↓ Z–A'}
        </button>
        <button
        ...
        {!loading && (
          <span className="ml-auto text-xs text-[var(--brand-muted)]">{displayedRows.length} row{displayedRows.length !== 1 ? 's' : ''}</span>
        )}
      </div>
