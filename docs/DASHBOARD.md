...
5. **Training — Live sessions**  
   Trainers create sessions; **one student share URL** uses `?session=<session_id>`. For multiple groups, students pick a group, then the app continues with `?group=<join_token>` (same join flow). Direct `?group=<token>` still works. Trainers list and delete their own sessions (`GET`/`DELETE` on `training-sessions`); admins may delete any. Public metadata: `public-training-session`; join/chat: `training-join`, `training-messages`, optional Realtime. **Session whiteboard:** optional per session via `training_sessions.whiteboard_enabled` (default true; trainer can disable when creating a session). In the React public join page, shared drawing syncs via Supabase Realtime **broadcast** on channel topic `public-live-{group_uuid}` (events include legacy `draw`, `clear-board`, and richer `wb-stroke` / `wb-line` / `wb-text`; no DB persistence). **Voice:** default room URLs are Jitsi (`JITSI_MEET_BASE`, default `https://meet.jit.si`, room `SBS-Session-{sessionId}` from `training-sessions`); the React join page embeds Jitsi in-page via External API (`prejoinConfig.enabled: false` after the in-app name step). The page shows an **SBS-branded strip** (logo + “Joined as …”) above the embed and passes the same logo as **`userInfo.avatarURL`** for the local Jitsi tile. The embed uses **audio-first** mode and a **minimal toolbar** (mic + hangup). **Limitation:** anything drawn inside the Jitsi iframe (for example “waiting for moderator” or login prompts on some public servers) is controlled by that Jitsi deployment, not by our CSS; after join we best-effort call `toggleLobby(false)`. For full control over lobby/moderator rules and chrome, point `JITSI_MEET_BASE` to a **self-hosted Jitsi** you configure. **Polls** use broadcast events `poll-start`, `poll-vote`, `poll-end` on the same channel (ephemeral). Participants detect a removed session by polling `public-training-session`. Apply migration `20260404_training_session_whiteboard_flag.sql` if the column is missing.
