...
- **Training** and **Operations → Import** are implemented in React (no iframe). They call the same Netlify functions as legacy (`training-sessions`, `training-data`, `classroom-data`, `course-library-data`, `credential-center`, `operations-data?bulk=1`, etc.). **Training → Assignments** (`/spa/training/assignments`) covers staff assignment CRUD, trainer attachment uploads (`classroom-assignment-upload` + `assignment-files`), and submission review (`submissions` PATCH). **`classroom-data?resource=pending-submissions`** powers the home dashboard queue for trainers/admins. **LMS / assessments:** `lms-analytics` and read-only `lms-admin-data` GETs under **Training → LMS analytics / LMS catalog**; full **`lms-admin-data`** CRUD under **Operations → LMS admin**; **`assessment-data`** under **Training → Assessments**; **`integration-events`** under **Operations → Integration events**.
